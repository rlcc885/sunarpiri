	...
	<div class="innertube">
		...
		<div id="navMenu">
			<div id="Explorer" style="visibility: visible">			
					...
					<%}%>
					<!-- copia de 18 =  editar datos personales -->
					<%
						System.out.println("arrp[93]-->"+arrp[93]);
						System.out.println("perfilusuarioid -->" + perfilusuarioid);
						System.out.println("Constantes.PERFIL_ADMIN_ORG_EXT-->"+Constantes.PERFIL_ADMIN_ORG_EXT);
					%>
					<%if(arrp[93]!=null && (perfilusuarioid != Constantes.PERFIL_ADMIN_ORG_EXT)) {%>
					  ...
					  <ul>       
						  <layer id=OutN2details visibility="show" LEFT=0px POSITION= relative> 
						    <div id=Out2details style="display:0; LEFT: 0px; POSITION: relative"> 
								<%if(arrp[93]!=null){PermisoBean p = (PermisoBean) arrp[93];%>    
					            	<li>
						           		<a href="<%=p.getUrl()%>" target="main_frame1" onmouseover="javascript:mensaje_status('<%=p.getDesc()%>');return true;" onmouseOut="javascript:mensaje_status(' ');return true;"><%=p.getDesc()%></a>
						          	</li>
								<%}%>
						...
						<ul>
							  <layer id=OutN3details visibility="show" LEFT=0px POSITION= relative> 
							  	<div id=Out3details style="display: 0; LEFT: 0px; POSITION: relative"> 
						          	<%if(arrp[94]!=null){
							  			PermisoBean p = (PermisoBean) arrp[94];%>
							            <li>
							           		<a href="<%=p.getUrl()%>" target="main_frame1" onmouseover="javascript:mensaje_status('<%=p.getDesc()%>');return true;" onmouseOut="javascript:mensaje_status(' ');return true;"><%=p.getDesc()%></a>
							          	</li>
						      		<%}%> 
