...
{	
	...
	//************************************************************************************
	document.frm1.action="/iri/Certificados.do?state=guardarDatosBasicos&hidTipo=B&flagCertBusq=<%=flag%>&criterio=<%=request.getAttribute("criterioBusqueda")%>&flagBusq=I";
	document.frm1.submit();
}
</script>
...
<tr>
  ...
  <tr class=grilla2>
	...
	<%if(certi){%>
	<td align="center">
	  <a href="/iri/Certificados.do?state=guardarDatosBasicos&refnum_part=<%=bean.getRefNumPart()%>&noPartida=<%=bean.getNumPartida()%>&hidOfic=<%=bean.getOficRegDescripcion()%>&hidTipo=L">
