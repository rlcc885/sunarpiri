...
System.out.println("perfilId-------------------------------->"+usuarioBean.getPerfilId());
System.out.println("Perfiles-->PERFIL_CAJERO-->"+Constantes.PERFIL_CAJERO + "-->PERFIL_ADMIN_ORG_EXT-->" + Constantes.PERFIL_ADMIN_ORG_EXT + "-->PERFIL_AFILIADO_EXTERNO-->" + Constantes.PERFIL_AFILIADO_EXTERNO + "-->PERFIL_INDIVIDUAL_EXTERNO-->" + Constantes.PERFIL_INDIVIDUAL_EXTERNO + "-->usuarioBean.getExonPago-->" + usuarioBean.getExonPago());
...
function VerPartida(refnum_Part,estado)
{
  ...
  if(doyou == true) {
	ventana=window.open('<%=request.getContextPath()%>/PublicidadIRI.do?state=visualizaPartida&refnum_part=' + refnum_Part,'1024x768','toolbar=no,status=yes,scrollbars=yes,resizable=yes,location=no,menubar=no,directories=no,width=950,height=650, top=0, left=0');
  }
}
...
<br>
<font color="#949400" size="2"><b>Resultado de la Consulta de Partidas por N&uacute;mero <%=request.getAttribute("outNumber")%></b></font>
<br>
 	...
 	<% if(usuarioBean.getPerfilId()==Constantes.PERFIL_CAJERO || usuarioBean.getPerfilId()==Constantes.PERFIL_INDIVIDUAL_EXTERNO){ %> 	
	  	<%if(flag!=null){
	  	 ...
	  	 %>
	  	  ...
	  	  <div id="HOJA2"> 
	  	  	<input type="button" class="formbutton" onclick="javascript:enviar();" value="Solicitar" onmouseover="javascript:mensaje_status('Solicitar Certificado');return true;" onmouseOut="javascript:mensaje_status(' ');return true;"/>
	  	  </div></td>
	  	 <%}}}%>
		<td width="50%" align="right">
		<div id="HOJA3">	
