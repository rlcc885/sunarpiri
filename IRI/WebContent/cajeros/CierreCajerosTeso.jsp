...
function Cerrar(){
		...
		var monto= Math.round(document.frm1.txtMontoBilletaje.value*100)/100;
			...
			if (p == true)
			document.frm1.action="/iri/Caja.do?state=cierreCaja";
			document.frm1.hid1.value = <%=request.getAttribute("usuarioCaja")%>;
...
function Cancelar(){
	document.frm1.action="/iri/Caja.do?state=aperturaCierreInicio";
	document.frm1.submit();
}
 ...
 var totalBilletes = Math.round( (montoDoscientos + montoCien + montoCincuenta + montoVeinte + montoDiez)*100)/100 ;
 ...
 var saldoFinal = Math.round((recaudEfectivo + recaudCheque)*100)/100;
 document.frm1.txtSaldoFinal.value = saldoFinal;
...
<input type="hidden" value="" name="hid1">
<table  class="grilla" >
				<!--<tr>
		      		<td colspan="6"></td>
		    	</tr>-->
		    	...
		    	<IMG border="0" src="images/btn_cerrar.gif" >
		    	<A href="javascript:Cancelar();" onmouseover="javascript:mensaje_status('Cerrar');return true;" onmouseOut="javascript:mensaje_status(' ');return true;">
		    	<IMG border="0" src="images/btn_cancelar.gif" ></div></td>
		    	</tr>		    	
