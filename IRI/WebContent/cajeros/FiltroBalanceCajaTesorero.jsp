...
<script language="JavaScript">
	...
	function buscar(){
		var codCaja = document.getElementById("selectCaja").value;
		if(codCaja == '-'){
		...
		document.form1.action="/iri/BalanceCajaTesorero.do?state=buscar";
		document.form1.submit();
	}

	function refreshFiltro(){
		var codEdificio = document.getElementById("selectEdificio").value;
		if(codEdificio == '-'){
			return;
		}
		document.form1.method="POST";
		document.form1.action="/iri/BalanceCajaTesorero.do?state=refresh";
		document.form1.submit();
	}
...
java.util.List listaCajas = (java.util.List) request.getAttribute("listaCajas");
...
<table border="0" class="tablasinestilo" cellspacing=0 >
  ...
  <tr>
  	<td width="10"></td>
    <td colspan="4"><strong>Edificio</strong></td>
    <td colspan="3"><strong>Caja</strong></td>
    <td valign="middle" width="59"></td>
  </tr>
  <tr>
  	<td width="10"></td>
    <td colspan="4">
	  <% String codigoEdificioSelected = (String)request.getAttribute("selectedEdificio");%>
	  <select size="1" name="edificio" id="selectEdificio" onchange="refreshFiltro();">
		    <logic:iterate name="listaEdificios" id="edificio" scope="request">
		    	<logic:equal name="edificio" property="codigo" value="<%=codigoEdificioSelected%>">
			        <option value="<bean:write name="edificio" property="codigo"/>" selected="selected"><bean:write name="edificio" property="descripcion"/></option>
		    	</logic:equal>
		    	<logic:notEqual name="edificio" property="codigo" value="<%=codigoEdificioSelected%>">
    ...
    <td colspan="3">
	  <select size="1" name="caja" id="selectCaja">
		    <logic:iterate name="listaCajas" id="caja" scope="request">
		        <option value="<bean:write name="caja" property="codigo"/>"><bean:write name="caja" property="descripcion"/></option>
		    </logic:iterate> 
      </select>
    </td>
