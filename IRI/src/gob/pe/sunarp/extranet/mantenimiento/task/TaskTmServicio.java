...
public class TaskTmServicio extends GrandTask
{
public TaskTmServicio()
{
	super();
	...
	this.ruta = "frmTmServicio";
}

public FormOutputListado getList(int pag) throws Throwable
{
DboTmServicio dbo = new DboTmServicio(conn);
...
for (int i = 0; i < arri.size(); ++i )
	{
		DboTmServicio d = (DboTmServicio) arri.get(i);
		...
		//_llaves
		gbean.setLlave01(d.getField(d.CAMPO_SERVICIO_ID));
		//_
		res.add(gbean);
	}
...
public void update(GenericBean bean) throws Throwable
{
	...
	dbo.setField(dbo.CAMPO_FG_MULT,  bean.getValor04());
...
public void insert(GenericBean bean) throws Throwable
{
	DboTmServicio dbo = new DboTmServicio(conn);
	/*
	dbo.setField(dbo.CAMPO_SERVICIO_ID,bean.getValor01());
	if (dbo.find())
