...
public class TaskTmGiro extends GrandTask
{
public TaskTmGiro()
{
	super();
	this.nombreTabla = "Giros";
...
{
DboTmGiro dbo = new DboTmGiro(conn);
...
for (int i = 0; i < arri.size(); ++i )
	{
		DboTmGiro d = (DboTmGiro) arri.get(i);
		...
		//_llaves
		gbean.setLlave01(d.getField(d.CAMPO_GIRO_ID));
		//_
		res.add(gbean);
	}
...
{
	DboTmGiro dbo = new DboTmGiro(conn);
	dbo.setField(dbo.CAMPO_GIRO_ID,bean.getLlave01());
	if (dbo.find()==false)
		throw new IllegalArgumentException("Error accediendo a base de datos");
	
	bean.setValor01(dbo.getField(dbo.CAMPO_GIRO_ID));
...
public void insert(GenericBean bean) throws Throwable
{
	DboTmGiro dbo = new DboTmGiro(conn);
	/*
	dbo.setField(dbo.CAMPO_GIRO_ID,bean.getValor01());
