...
public class TaskTmTipoComb extends GrandTask
{
public TaskTmTipoComb()
{	
	super();
	this.nombreTabla = "Tipo de Combustible";
	this.nKeys = 1;  //numero de campos llave
	this.cabezas = new String[] {
						"Código",
						"Descripción"						
					};	
	this.ruta = "frmTmTipoComb";
}


public FormOutputListado getList(int pag) throws Throwable
{
DboTmTipoComb dbo = new DboTmTipoComb(conn);

dbo.setMaxRecords(Propiedades.getInstance().getLineasPorPag());
ArrayList arri = dbo.searchAndRetrieveListPaginado(dbo.CAMPO_COD_TIPO_COMB, pag);
...
{
	DboTmTipoComb dbo = new DboTmTipoComb(conn);
	dbo.setField(dbo.CAMPO_COD_TIPO_COMB,bean.getLlave01());
	if (dbo.find()==false)
		throw new IllegalArgumentException("Error accediendo a base de datos");
	
	bean.setValor01(dbo.getField(dbo.CAMPO_COD_TIPO_COMB));
...
{
	DboTmTipoComb dbo = new DboTmTipoComb(conn);
	dbo.setField(dbo.CAMPO_COD_TIPO_COMB,bean.getValor01());
	if (dbo.find())
