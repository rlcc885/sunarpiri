 ...
 *
 */
public class BalanceCajaTesoreroController extends ControllerExtension {

	private String thisClass = BalanceCajaTesoreroController.class.getName() + ".";

	public BalanceCajaTesoreroController() {
		super();
	...
	protected ControllerResponse runMuestraState (
		...
		try{
			...
			if(listaEdificios.isEmpty()){
				ComboBean comboBeanAux = new ComboBean();
				comboBeanAux.setCodigo("-");
				comboBeanAux.setDescripcion("----------");
				listaEdificios.add(comboBeanAux);
			}
			req.setAttribute("listaEdificios", listaEdificios);
			UsuarioBean usuario = ExpressoHttpSessionBean.getUsuarioBean(request);
			String codigoEdificio = ((ComboBean)listaEdificios.get(0)).getCodigo();
			List listaCajas = Tarea.getComboCajasPorEdificio(dconn, usuario.getRegPublicoId(), codigoEdificio);
			if(listaCajas.isEmpty()){
				ComboBean comboBeanAux = new ComboBean();
				comboBeanAux.setCodigo("-");
				comboBeanAux.setDescripcion("----------");
				listaCajas.add(comboBeanAux);
			}
			req.setAttribute("listaCajas", listaCajas);
			req.setAttribute("selectedEdificio", codigoEdificio);
	...
	protected ControllerResponse runRefreshState (
		ControllerRequest request,
		ControllerResponse response)
		throws ControllerException {
		...
		try{
			...
			req.setAttribute("listaEdificios", listaEdificios);
			UsuarioBean usuario = ExpressoHttpSessionBean.getUsuarioBean(request);
			String codigoEdificio = request.getParameter("edificio");
			List listaCajas = Tarea.getComboCajasPorEdificio(dconn, usuario.getRegPublicoId(), codigoEdificio);
			if(listaCajas.isEmpty()){
				ComboBean comboBeanAux = new ComboBean();
				comboBeanAux.setCodigo("-");
			...
			String diaSelected = request.getParameter("dia");
			String mesSelected = request.getParameter("mes");
			String anioSelected = request.getParameter("anio");
			req.setAttribute("selectedDay", diaSelected);
			req.setAttribute("selectedMonth", mesSelected);
			req.setAttribute("selectedYear", anioSelected);
	...
	protected ControllerResponse runBuscarState (
		...
		try{
			...
			String codigoEdificio = request.getParameter("edificio");
			String codigoCaja = request.getParameter("caja");
	 ...
	 * @return
	 */
	private String obtenerUserIdCajero(DBConnection dconn, String codigoCaja, List listaCajas) {
		String descCaja = "";
		Iterator iterator = listaCajas.iterator();
		while (iterator.hasNext()) {
			ComboBean element = (ComboBean) iterator.next();
			if(element.getCodigo().equals(codigoCaja)){
				return
	...
	private String obtenerTotalIngresoCajaEfectivo(String userId, String fechaInicio, String fechaFin, DBConnection dconn) throws DBException {
		...
		String estadoNoExtornado = "1";//agregado jbugarin
		...
		/** modificado para que no incluya los extornos en el reporte de balance de cajas**/
