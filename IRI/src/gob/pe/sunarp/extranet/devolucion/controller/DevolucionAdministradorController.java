 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class DevolucionAdministradorController extends ControllerExtension implements Constantes{
	private String thisClass = DevolucionController.class.getName() + ".";

		public DevolucionAdministradorController() {
			super();
	...
	protected ControllerResponse runBuscaSolicitudDevState(ControllerRequest request,ControllerResponse response) throws ControllerException{
		...
		try{
			...
			while(iterator.hasNext()) {
				...
				solicitud.setFechaInforme(fechaInforme.equals("") ? "---" : FechaUtil.expressoDateToUtilDate(fechaInforme).replace('/','.'));
