...
public class ConsultaIndicePartidasxTipoNumeroDocumentoServiceImpl extends ServiceImpl implements ConsultaIndicePartidasxTipoNumeroDocumentoService, Constantes{
	private Connection conn;
	private DBConnection dbConn;
	private DBConnectionFactory pool;
	
	public ConsultaIndicePartidasxTipoNumeroDocumentoServiceImpl() throws Exception{
		this.pool = DBConnectionFactory.getInstance();
		this.conn = pool.getConnection();
		this.dbConn = new DBConnection(conn);
	}
	
	public ConsultaIndicePartidasxTipoNumeroDocumentoServiceImpl(DBConnectionFactory pool, Connection conn, DBConnection dbConn){
		this.conn = conn;
		this.dbConn = dbConn;
		this.pool = pool;
	}
	
	public FormOutputBuscarPartida busquedaIndiceTipoNumeroDocumentoRMC(int medioDeAcceso,InputBusqIndirectaBean inputBusqIndirectaBean ,UsuarioBean usuario ,String ipOrigen, String amid_session) throws SQLException, CustomException, ValidacionException, DBException, Throwable{
		
        FormOutputBuscarPartida formOutputBuscarPartida = null;
		try{
			
			ConsultaIndicePartidasTipoNumeroDocumentoSQL consultaIndicePartidasTipoNumeroSQL = new ConsultaIndicePartidasTipoNumeroDocumentoSQLImpl(this.conn, this.dbConn);
			//Inicio:mgarate:27/08/2007
			//se agrega el medio de acceso para evitra la paginacion en los web service
			formOutputBuscarPartida = consultaIndicePartidasTipoNumeroSQL.busquedaIndiceTipoNumeroDocumentoRmc(medioDeAcceso,inputBusqIndirectaBean);
			//Fin:mgarate
			//Tarifa								
			double tarifa = 0;
			...
			if (inputBusqIndirectaBean.getFlagPagineo()==false)
			{
				...
				bt.setPartSeleccionado("TND"); // TipoNumeroDocumento
				bt.setTipoBusqPartida(busqpartida);
				bt.setTipoDocumento(inputBusqIndirectaBean.getTipoDocumento());
				bt.setNumeroDocumento(inputBusqIndirectaBean.getNumeroDocumento());
