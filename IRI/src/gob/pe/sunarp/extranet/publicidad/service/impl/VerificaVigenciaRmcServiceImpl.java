...
public class VerificaVigenciaRmcServiceImpl extends ServiceImpl implements VerificaVigenciaRmcService {
	private Connection conn;
	private DBConnection dbConn;
	
	public VerificaVigenciaRmcServiceImpl(Connection conn, DBConnection dbConn){
		this.conn = conn;
		this.dbConn = dbConn;
	}
public ArrayList constanciaTituloCertificadoVigenciaRmc(ObjetoSolicitudBean objetoSolicitudBean) throws SQLException, CustomException, ValidacionException, DBException, Throwable{
		...
		try{
			...
			VerificaVigenciaRmcSQL verificaVigenciaRmcSQL = new VerificaVigenciaRmcSQLImpl(this.conn, this.dbConn);
			if(objetoSolicitudBean.getTpo_pers()!=null&&objetoSolicitudBean.getTpo_pers().trim().equalsIgnoreCase(Constantes.PERSONA_NATURAL)){
				listaPartidas = verificaVigenciaRmcSQL.busquedaPersonaNaturalVigenciaRMC(objetoSolicitudBean.getNombres(), objetoSolicitudBean.getApe_pat(),objetoSolicitudBean.getApe_mat());
			}else
				if(objetoSolicitudBean.getTpo_pers()!=null && objetoSolicitudBean.getTpo_pers().trim().equalsIgnoreCase(Constantes.PERSONA_JURIDICA)){
					listaPartidas = verificaVigenciaRmcSQL.busquedaPersonaJuridicaVigenciaRMC(objetoSolicitudBean.getRaz_soc(), objetoSolicitudBean.getSiglas());
				}else
					if(objetoSolicitudBean.getTpo_pers()!=null && objetoSolicitudBean.getTpo_pers().trim().equalsIgnoreCase(Constantes.PERSONA_TIPO_DOCUMENTO)){
						listaPartidas = verificaVigenciaRmcSQL.busquedaTipoNumeroDocVigenciaRMC(objetoSolicitudBean.getTipoDocumento(),objetoSolicitudBean.getNumeroDocumento());
					}
