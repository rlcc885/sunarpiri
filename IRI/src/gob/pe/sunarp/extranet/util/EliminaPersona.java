...
/*
Enero 2003
Proceso para eliminar físicamente registros de la tabla PERSONA
y además borrar sus usuarios del TAM
*/	
...
try{
	...
	String rp = ruta+"//WEB-INF//log//eliminaPersona.txt";
	fw = new FileWriter(rp, true);
	bw = new BufferedWriter(fw);
	
	mensaje("******************************************************************");
	mensaje("Inicia Proceso elimina Persona "+FechaUtil.getCurrentDateTime());
	...
	finally
	{
		...
		mensaje("Fin Proceso elimina Persona "+FechaUtil.getCurrentDateTime());
		pool.release(conn);
		try{
		bw.close();
		fw.close();
